...
class ForecastWriteSerializer(serializers.ModelSerializer):
    ...
    def multiple_choice_validation(
        self,
        probability_yes_per_category: dict[str, float],
        ...
        for option in all_options:
            if option in probability_yes_per_category:
                value = probability_yes_per_category[option]
                if not (0.001 <= value <= 0.999):
                    raise serializers.ValidationError(
                        "All probabilities must be between 0.001 and 0.999"
                    )
            else:
                value = 0.0
            values.append(value)
        if not np.isclose(sum(values), 1):
            raise serializers.ValidationError(
