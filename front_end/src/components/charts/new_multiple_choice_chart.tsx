...
const NewMultipleChoiceChart: FC<Props> = ({
  timestamps,
  actualCloseTime,
  choiceItems,
  ...
  const userTimestamps = onCursorChange
    ? choiceItems[0]?.userTimestamps ?? []
    : [];
  const userCursorTimestamp = userTimestamps[userCursorIndex ?? 0];
  return (
      ...
      >
        {shouldDisplayChart && (
          ...
          >
            ...
            {userCursorTimestamp && isCursorActive && (
              <VictoryScatter
                data={[
                  { x: userCursorTimestamp, y: 0.5, symbol: "circle", size: 4 },
                ]}
                dataComponent={
                  <MyPredictionChip
                    x={userCursorTimestamp}
                    chartTopPadding={TOP_PADDING}
                  />
                }
              />
            )}
...
const MyPredictionChip: FC<{
  ...
  const CHIP_LINE_WIDTH = 23;
  const TEXT_PADDING = 45;
  const { x, y, scale, chartTopPadding } = props;
  const { getThemeColor } = useAppTheme();
  const t = useTranslations();
  ...
  return (
    <g>
      ...
      <line
        x1={scale.x(x) - CHIP_LINE_WIDTH}
        x2={scale.x(x) + CHIP_LINE_WIDTH}
        y1={chartTopPadding}
        y2={chartTopPadding}
...
const PredictionRectangle: React.FC<PredictionRectangleProps> = (props) => {
  ...
  // Calculate positions directly in SVG space
...
export default memo(NewMultipleChoiceChart);
