...
import { aggregateServicesQuizAnswers } from "../../helpers";
...
export const ServicesQuizFlowProvider: FC<
  PropsWithChildren<{ onSubmit?: () => Promise<void> | void }>
> = ({ onSubmit, children }) => {
  ...
  const goNext = useCallback(async () => {
    ...
    if (step === 5) {
      const aggregated = aggregateServicesQuizAnswers(state);
      console.log("[services-quiz] submit payload:", aggregated);
      await onSubmit?.();
      setStep(6);
      return;
    }
    ...
    setStep((s) => (s + 1) as ServicesQuizStepId);
  }, [canGoNext, nextDisabled, step, onSubmit, state]);

  const selectStep = useCallback((id: number | string) => {
