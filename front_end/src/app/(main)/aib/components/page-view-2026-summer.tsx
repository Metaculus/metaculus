...
import { DatesSummer2026, PrizeSummer2026 } from "./cards-2026-summer";
import Description from "./description";
import HeroSummer2026 from "./hero-2026-summer";
import TournamentPager, { TOURNAMENT_ITEMS } from "./tournament-pager";

type Props = {
  primaryBotToken?: string | null;
};

const AiBenchmarkingTournamentPageSummer2026: FC<Props> = ({
  primaryBotToken,
}) => {
  const { user } = useAuth();
      ...
      <div className="flex size-full flex-col items-center gap-3">
        <div className="flex w-full flex-col gap-3 md:flex-row">
          ...
          <div className="relative flex h-auto min-h-[8rem] w-full flex-row overflow-hidden rounded md:w-1/3 lg:h-auto">
            <Image
              src="https://cdn.metaculus.com/hires-summer.webp"
              alt=""
              fill
              priority
              sizes="100vw"
          ...
          <div className="flex h-auto w-full flex-col gap-3 md:w-1/3">
            {!isUserAuthenticated && (
              <div className="flex h-auto w-full grow flex-col items-center justify-center gap-2 rounded bg-blue-800 p-8 text-white dark:bg-blue-800-dark dark:text-blue-900 lg:px-12">
                ...
                <div>{t("FABRegisterBotSecondary")}</div>
              </div>
            )}
            {isUserAuthenticated && !primaryBotToken && (
              <div className="flex h-auto w-full grow flex-col items-center justify-center gap-2 rounded bg-blue-800 p-8 text-white dark:bg-blue-800-dark dark:text-blue-900 lg:px-9">
                <span className="text-center text-xl uppercase tracking-wide opacity-50 min-[1920px]:text-2xl">
                  {t("FABGettingStarted")}
                </span>
                <span className="mb-2 text-center text-3xl min-[1920px]:text-2xl">
                  {t("FABCreateBotTitle")}
                </span>
                <Button
                  variant="secondary"
                  ...
                  size="lg"
                  className="border-none"
                  href="/accounts/settings/bots/#create"
                >
                  {t("createBot")}
                </Button>
            ...
            <div className="flex flex-col gap-3">
              ...
              <Link
                href="/tournament/summer-aib-2026/"
                className="flex w-full items-center gap-4 rounded bg-white p-4 no-underline transition-all hover:bg-blue-500/40 dark:bg-blue-100-dark dark:hover:bg-blue-600/40 md:p-5 min-[1920px]:p-6"
              >
                <FontAwesomeIcon
                  icon={faTrophy}
...
export default AiBenchmarkingTournamentPageSummer2026;
