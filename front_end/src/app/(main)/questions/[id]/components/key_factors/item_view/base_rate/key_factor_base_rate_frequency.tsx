  ...
  return (
    <div>
      <div className="flex items-center gap-1.5 text-base text-gray-500">
        <span className="text-2xl font-normal text-purple-800">
          {numerator}
        </span>
        <span>in</span>
      ...
      <FrequencyBoxes numerator={numerator} denominator={denominator} />
    </div>
  );
};
...
const Box: React.FC<{
  ...
  if (kind === "filled")
    return (
      <span
        className={cn(classBase, "bg-purple-600")}
  ...
  return (
    <span
      style={{ width: base, height: base }}
      className={cn(classBase, "bg-gray-200")}
    />
  );
};
