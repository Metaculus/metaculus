...
from django.db.models import When, Case, Value, Q, F, Subquery, OuterRef
...
def migrate(apps, schema_editor):
    ...
    # setting up for depreicating bot_leaderboard_status on Project model, so
    # we copy the value to the leaderboard before populating the leaderboard's
    # bot/human_exclusion_status
    project_bot_status = Project.objects.filter(pk=OuterRef("project_id")).values(
        "bot_leaderboard_status"
    )[:1]
    Leaderboard.objects.filter(bot_status__isnull=True).update(
        bot_status=Subquery(project_bot_status)
    )
