...
class TestQuestionForecast:
    ...
    def test_forecast_binary(self, question_binary, user1, user1_client, props):
        response = user1_client.post(
            self.url,
            data=json.dumps([{"question": question_binary.id, **props}]),
            content_type="application/json",
        )
        assert response.status_code == 201
        forecast = Forecast.objects.filter(
            question=question_binary, author=user1
        ).first()
        assert forecast
    ...
    def test_forecast_binary_invalid(self, question_binary, user1_client, props):
        response = user1_client.post(
            self.url,
            data=json.dumps([{"question": question_binary.id, **props}]),
            content_type="application/json",
    ...
    def test_forecast_multiple_choice(
        self, question_multiple_choice, user1, user1_client, props
    ):
        response = user1_client.post(
            self.url,
            data=json.dumps([{"question": question_multiple_choice.id, **props}]),
            content_type="application/json",
        )
        assert response.status_code == 201
        forecast = Forecast.objects.filter(
            question=question_multiple_choice, author=user1
        ).first()
        assert forecast
    ...
    def test_forecast_multiple_choice_invalid(
        self, question_multiple_choice, user1_client, props
    ):
        response = user1_client.post(
            self.url,
            data=json.dumps([{"question": question_multiple_choice.id, **props}]),
    ...
    def test_forecast_numeric(self, question_numeric, user1, user1_client, props):
        response = user1_client.post(
            self.url,
            data=json.dumps([{"question": question_numeric.id, **props}]),
            content_type="application/json",
        )
        assert response.status_code == 201
        forecast = Forecast.objects.filter(
            question=question_numeric, author=user1
        ).first()
        assert forecast
    ...
    def test_forecast_discrete(self, question_discrete, user1, user1_client, props):
        response = user1_client.post(
            self.url,
            data=json.dumps([{"question": question_discrete.id, **props}]),
            content_type="application/json",
        )
        assert response.status_code == 201
        forecast = Forecast.objects.filter(
            question=question_discrete, author=user1
        ).first()
        assert forecast
        assert forecast.continuous_cdf == props.get("continuous_cdf")
    ...
    def test_forecast_numeric_invalid(self, question_numeric, user1_client, props):
        response = user1_client.post(
            self.url,
            data=json.dumps([{"question": question_numeric.id, **props}]),
            content_type="application/json",
    ...
    def test_forecast_numeric_boundary_test(
        self, question_numeric, user1_client, status_code, q_props, f_props
    ):
        create_question(question=question_numeric, **q_props)
        response = user1_client.post(
            self.url,
            data=json.dumps([{"question": question_numeric.id, **f_props}]),
            content_type="application/json",
        )
...
class TestQuestionResolve:
    ...
    def test_resolve_binary(self, question_binary, user_admin_client):
        url = reverse("question-resolve", args=[question_binary.id])
        ...
        question = Question.objects.get(id=question_binary.id)
    ...
    def test_resolve_multiple_choice(self, question_multiple_choice, user_admin_client):
        url = reverse("question-resolve", args=[question_multiple_choice.id])
        response = user_admin_client.post(
            url,
            data=json.dumps(
                {
        ...
        assert response.status_code == 200
        question = Question.objects.get(id=question_multiple_choice.id)
    ...
    def test_resolve_continuous(
        self, question_numeric, user_admin_client, status_code, q_props, resolution
    ):
        question = create_question(question=question_numeric, **q_props)
        url = reverse("question-resolve", args=[question.id])
        response = user_admin_client.post(
            url,
            data=json.dumps(
                {
